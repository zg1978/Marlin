...
uint8_t Language_Mode = 0;
uint8_t FILA_SENSOR_FLAG = 0;
...
void lcd_sdcard_pause() {
  ...
  lcd_setstatusPGM(PSTR(MSG_PRINT_PAUSED), -1);
    ...
    void lcd_sdcard_stop() {
      ...
      #if FAN_COUNT > 0
        for (uint8_t i = 0; i < FAN_COUNT; i++) fanSpeeds[i] = 255; ///The fan always open when stop print 20180330
      #endif
...
//		if(Language_Mode == 1)
//		{
//			MENU_ITEM(submenu, "\xa4\xa5", lcd_prepare_menu);
//		}
     // MENU_ITEM(submenu, MSG_PREPARE, lcd_prepare_menu);
     // MENU_ITEM(submenu, "prepareCN", lcd_prepare_menu);
    }
    MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);
    //MENU_ITEM(submenu, "Language", lcd_language_menu);  ///20180119
    #if ENABLED(SDSUPPORT)
	MENU_ITEM(gcode, "Init_SDCard", PSTR("M21"));
      if (card.cardOK) {
        if (card.isFileOpen()) {
          if (card.sdprinting)
            MENU_ITEM(function, MSG_PAUSE_PRINT, lcd_sdcard_pause);
...
MENU_ITEM(gcode, "Load Filament", PSTR(
  "G28\n"
  "G1 Z50 F200\n"
  "M109 S210\n"
  "G92 E0\n"
  ...
  MENU_ITEM(function, FIRMWARE_VERSION, firmware_version);
...
//  static void lcd_English_settings(){
    //btDefult=&btEnglish;
 //   Language_Mode = 0;
//     lcd_completion_feedback();
//    }

 // static void lcd_Chinese_settings(){
    //btDefult=&btChinese;
//    Language_Mode = 1;
 //    lcd_completion_feedback();
//    }
  ...
  static void deal_FIA_SENSOR()
  {
    if(FILA_SENSOR_FLAG == 1)
    {
      FILA_SENSOR_FLAG = 0;
    }else{
      FILA_SENSOR_FLAG = 1;
  ...
  void lcd_control_menu() {
    ...
    MENU_ITEM(function, MSG_RESTORE_FAILSAFE, lcd_factory_settings);
      ...
      //MENU_ITEM(submenu, MSG_INIT_EEPROM, lcd_init_eeprom_confirm);
        if(FILA_SENSOR_FLAG == true)
        {
          MENU_ITEM(function, "FILAMENT SENSOR ON", deal_FIA_SENSOR);
        }else{
          MENU_ITEM(function, "FILAMENT SENSOR OFF", deal_FIA_SENSOR);
    }
    #endif
 ...
 // void lcd_language_menu()
  ...
  //  START_MENU();
  //  MENU_BACK(MSG_MAIN);
  //  MENU_ITEM(function, "English", lcd_English_settings);
  //  MENU_ITEM(function, "Chinese", lcd_Chinese_settings);
  //  END_MENU();
 //   }
  /**
   *
