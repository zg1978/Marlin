...
extern uint8_t FILA_SENSOR_FLAG;
...
#if FAN_COUNT > 0
  int16_t fanSpeeds[FAN_COUNT] = { 255 }; ///20180318
  #if ENABLED(PROBING_FANS_OFF)
    bool fans_paused = false;
    int16_t paused_fanSpeeds[FAN_COUNT] = { 255 };  ///20180318 
  #endif
#endif
...
inline void gcode_M105() {
  ...
  inline void gcode_M106() {
    ...
    if (p < FAN_COUNT) fanSpeeds[p] = 255;  ///fanSpeeds[p] = s;
  }
  ...
  inline void gcode_M107() {
    ...
    if (p < FAN_COUNT) fanSpeeds[p] = 255;  ///20180318
  }
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/) {
  ...
  #if ENABLED(FILAMENT_RUNOUT_SENSOR)
    if ((IS_SD_PRINTING || print_job_timer.isRunning()) && (READ(FIL_RUNOUT_PIN) == FIL_RUNOUT_INVERTING) && (FILA_SENSOR_FLAG == 1))
